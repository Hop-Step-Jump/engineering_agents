...
| `electrolysis_temp` | 電解動作温度 (°C) | 100.0 |
| `sabatier_temp` | サバティエ反応温度 (°C) | 300.0 |
...
| --- | --- |
| ARS CO₂ 除去 | `scrubber_degradation`: `MockEclssSimulator`（スクラバー効率・ファン・バイパス）。`ssos_eclss_loop`: `EclssBackend`（ARS action / `request_co2`） |
| WRS / OGS | **接続済み**（`ssos_eclss_loop`）。`MockEclssBackend` / `Ros2EclssBridge` が OGS・WRS action と product/grey water service を実装（`SsosAdapter` は不使用） |
