...
| M | scrubber | ダッシュボード基準線 1000 ≠ ヘルス 800/1200 | Low | **fixed** |
| N | 文書 | OGS `sabatier_temp` を (K) と誤記（値 300 は °C 相当）；`electrolysis_temp` 単位欠落 | Low | open |
...
- `sabatier_temp` を **(K)** と記載しているが、デフォルト **300.0** は 300 K（≈27°C）では触媒温度として非現実。同文書の ARS/WRS 限界は °C → **°C** が妥当。
- `electrolysis_temp`（100.0）は単位なし → **°C** を明記すべき。
