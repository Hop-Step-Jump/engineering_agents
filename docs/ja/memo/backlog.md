...
| labeled が command failure を無視 | **修正済み（bug I）** — `operational_rejected` + フラグ再アーム |
...
### P1 — プラント初期状態（CO2=500g）

| 項目 | 説明 |
|------|------|
| `scenario.yaml` | `simulation.initial_co2_storage_g: 500`（mock 用。現状 1500） |
| ros2 step 0 | headless 再起動後の `/co2_storage` を `summary.plant_initial_co2_storage_g` に記録 |
| 検証 | `initial_co2_storage_g`（500）との許容差外で fail fast（SSOS launch 設定を案内） |
| テスト | `tests/scenario/test_ssos_eclss_loop.py` の CO2 期待値更新 |
| SSOS 側 | headless デフォルトが 500g でない場合、launch パラメータ調査（engineering_agents 単独では物理状態を捏造しない） |
...
| セクション | 内容 |
|-----------|------|
| Run メタ | `duration_wall_s`, `plant_initial_co2_storage_g`, ops 数 |
| 閾値ライン | CO2/O2 プロットに `co2_storage_high_g` / `o2_storage_low_g` |
...
## BL-008: ECLSS 既知バグ修正（単位・力学・閉ループ） { #bl-008 }

**ステータス**: 調査完了・修正は個別承認制  
**関連**: [known_bugs_inventory.md](ssos_eclss_loop/known_bugs_inventory.md)

ECLSS 異常対応シミュレーションにおける **単位不統一（g↔kg・°C 等）**、mock 力学の符号・非連動、エージェント／閉ループの既知ギャップを A–Q として整理した。| 優先 | 項目 | 説明 |
|------|------|------|
| P0 | **A** (**fixed**) / **E** (**fixed**) | ストレージ単位を g に統一済み；`request_co2` 符号逆も修正済み |
| P1 | **F / B / G / C** | 水テレメ非連動；mass↔L；goal 無視；単位契約 |
| P2 | **I** (**fixed**) / **J / H / L / M** / **D** (**fixed**) / **N** (**fixed**) | 失敗時 rejected 化済み；critical 未使用；初期値；scrubber 電力／基準線；製品水・OGS 温度単位は修正済み |
| P2 | **K** (**fixed**) | scrubber 設計提案の `--apply-proposals` 再注入 |
