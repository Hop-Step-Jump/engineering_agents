...
シミュレーション結果は JSONL に記録され、[Streamlit ダッシュボード](#ダッシュボード) で確認できます。`scrubber_degradation` は CO₂ ppm・EPS・トポロジ Before/After、`ssos_eclss_loop` はストレージ g・運用タイムライン・`ssos_graph` 設計提案を表示します。
...
| --- | --- | --- |
| 判断の源泉 | `policy` / 閾値（scrubber: CO₂ ppm、ssos: ストレージ g） | Persona + テレメトリ + チーム発言（**policy は読まない**） |
...
- **Overview** — 単一 run または 2 run 比較（scrubber: CO₂ ppm / EPS、ssos: ストレージ g）  
