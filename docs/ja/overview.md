...
シミュレーション結果は JSONL に記録され、[Streamlit ダッシュボード](#ダッシュボード) で確認できます。`scrubber_degradation` は CO₂ ppm・EPS・トポロジ Before/After、`ssos_eclss_loop` はストレージ（CO₂/O₂ は g、製品水は L）・運用タイムライン・`ssos_graph` 設計提案を表示します。
...
`ssos_eclss_loop` の run を選ぶと、ダッシュボードは **CO₂ / O₂（g）/ 製品水（L）** プロット、ヘルスカード（safe / warning / critical）、**運用タイムライン**（`air_revitalisation`、`oxygen_generation`、`request_co2` 等の `operational_applied`）を表示します。2 run 比較で labeled と LLM、または異なるモデルの運用タイミングの差を追えます。事後の `ssos_graph` 設計提案（`action_profile`、`graph_rewire`）も Step replay から確認できます。
...
| --- | --- | --- |
| 判断の源泉 | `policy` / 閾値（scrubber: CO₂ ppm、ssos: ストレージ g / L） | Persona + テレメトリ + チーム発言（**policy は読まない**） |
...
ルールベースは **「正しい挙動の足場」** です。LLM モードは、固定閾値では表現しきれない **状況理解・チーム合意・回復タイミングの違い** を観察するための実験モードです。LLM 設計で重要だった点は次のとおりです（詳細は [homogeneous agent team plan](memo/agents/homogeneous_agent_team_plan.md)）。
...
| **テレメトリ** | — | `/co2_storage`、`/o2_storage`、`/wrs/product_water_reserve`（g / L） |
| **運用コマンド** | — | ARS / OGS Action、`request_co2` / `request_o2` Service |
| **設計提案** | — | `ssos_graph`（`action_profile`、`graph_rewire` 等） |

**ヘルス閾値（ストレージ）**: CO₂ warning ≥ 1500 g / critical ≥ 2200 g；O₂ warning ≤ 450 g / critical ≤ 337.5 g。詳細は [scenario-ssos-eclss-loop.md](scenario-ssos-eclss-loop.md)。
...
`scrubber_degradation` を `labeled_rule_base` で実行します（Ollama 不要）。詳細は [docs/cli.md](cli.md)。
...
| ファイル | 説明 |
| --- | --- |
| `telemetry.jsonl` | CO₂/O₂/水ストレージ（g / L） |
...
| `summary.json` | `backend`、`peak_co2_storage_g`、`operational_command_count` 等 |
...
- **Overview** — 単一 run または 2 run 比較（scrubber: CO₂ ppm / EPS、ssos: ストレージ g / L）  
