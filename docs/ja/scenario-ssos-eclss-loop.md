...
| テレメトリ | CO₂ ppm、スクラバー効率、電力マージン | `/co2_storage`、`/o2_storage`、`/wrs/product_water_reserve`（g / L） |
...
| mock | 毎 step CO₂ が `co2_growth_g_per_step` だけ増加（デフォルト +60 g/step） |
...
| 条件（目安） | 運用コマンド |
| --- | --- |
| CO₂ ≥ `co2_storage_high_g`（デフォルト 1500 g） | `air_revitalisation`（ARS） |
| O₂ ≤ `o2_storage_low_g`（デフォルト 450 g） | 先に `request_co2`（policy 既定 ON）→ `oxygen_generation`（OGS） |
...
| --- | --- | --- | --- |
| CO₂ ストレージ (g) | < high（1500） | high 〜 critical 未満 | ≥ critical（2200） |
| O₂ ストレージ (g) | > low（450） | low×0.75 〜 low | ≤ low×0.75（337.5） |
| 製品水 (L) | > low（50） | low×0.5 〜 low | ≤ low×0.5（25） |
| `overall` | 全 safe | より悪い方 | より悪い方 |

エージェントの運用トリガー（`co2_storage_high_g` 等）は `scenario.yaml` の `thresholds`。ヘルス区分はテレメトリから独立に記録される。
...
| ファイル | いつ読むか |
| --- | --- |
| `telemetry.jsonl` | CO₂/O₂/水ストレージの時系列（g / L） |
...
| `peak_co2_storage_g` | ラン中最大 CO₂ ストレージ |
| `final_co2_storage_g` / `final_o2_storage_g` | 最終 step のストレージ |
