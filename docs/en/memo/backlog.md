...
## BL-008: ECLSS known bug fixes (units, dynamics, closed loop) { #bl-008 }
...
Inventory of **unit mismatches (g↔kg, °C, etc.)**, mock dynamics sign/sync bugs, and agent/closed-loop gaps for the ECLSS anomaly-response simulation (IDs A–Q).| Priority | Items | Notes |
|----------|-------|-------|
...
| P2 | **K** (**fixed**) | Scrubber `--apply-proposals` re-injection |
