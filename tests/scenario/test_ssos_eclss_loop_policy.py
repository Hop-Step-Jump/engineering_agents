...
def test_merge_labeled_policy_from_thresholds_copies_bands():
    ...
    thresholds = {
        "co2_storage_high_g": 1550.0,
        "o2_storage_low_g": 440.0,
        "product_water_low_l": 55.0,
    }
    merged = merge_labeled_policy_from_thresholds(agents_config, thresholds)
    assert merged["policy"]["co2_storage_high_g"] == 1550.0
    assert merged["policy"]["o2_storage_low_g"] == 440.0
...
def test_merge_labeled_policy_skips_llm_mode():
    agents_config = {"mode": "llm", "policy": {"co2_storage_high_g": 999.0}}
    merged = merge_labeled_policy_from_thresholds(
        agents_config,
        {"co2_storage_high_g": 1500.0},
    )
    assert merged["policy"]["co2_storage_high_g"] == 999.0
